...
const mockTransactionSuccessCounter = { inc: jest.fn() };
const mockTransactionFailureCounter = { inc: jest.fn() };
...
describe('TransactionService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: 'PROM_METRIC_TRANSACTION_SUCCESS_TOTAL', useValue: mockTransactionSuccessCounter },
        { provide: 'PROM_METRIC_TRANSACTION_FAILURE_TOTAL', useValue: mockTransactionFailureCounter },
      ],
    }).compile();
  ...
  describe('transferByEmail', () => {
    it('deve realizar uma transferência com sucesso', async () => {
      ...
      expect(mockTransactionSuccessCounter.inc).toHaveBeenCalledWith({ type: 'transfer' });
    ...
    it('deve lançar erro se saldo for insuficiente', async () => {
      ...
      expect(mockTransactionFailureCounter.inc).toHaveBeenCalledWith({ type: 'transfer' });
    });
    ...
    it('deve lançar erro se tentar transferir para si mesmo', async () => {
      ...
      (mockEntityManager.findOne as jest.Mock).mockImplementation(async (entity, options) => {
        if (entity === Wallet) {
          return { id: 'wallet-1', balance: 1000, currency: 'BRL', user: { id: 'user-1' } } as Wallet;
        }
        if (entity === User) {
          return { id: 'user-1', email: 'sameuser@mail.com' } as User;
        }
        return null;
      });
      await expect(
  ...
  describe('refundTransaction', () => {
    it('deve estornar com sucesso se for a transação do user e dentro do prazo, com saldo no toWallet', async () => {
      const mockTransaction = {
        ...
        createdAt: new Date(),
        amount: 100,
        fromWallet: { id: 'wallet-from', balance: 900, user: { id: 'user-1' } },
        toWallet: { id: 'wallet-to', balance: 500, user: { id: 'user-2' } },
      ...
      expect(mockEntityManager.save).toHaveBeenCalledTimes(2);
      expect(mockQueryRunner.commitTransaction).toHaveBeenCalled();
      expect(mockTransactionSuccessCounter.inc).toHaveBeenCalledWith({ type: 'refund' });
    ...
    it('deve lançar erro se a carteira de origem for inválida', async () => {
      const mockTransaction = { fromWallet: null } as any as Transaction;
      (mockEntityManager.findOne as jest.Mock).mockResolvedValueOnce(mockTransaction);
      await expect(
    ...
    it('deve lançar erro se o user não for dono da transação', async () => {
      const mockTransaction = { fromWallet: { user: { id: 'outro-user' } } } as any as Transaction;
      (mockEntityManager.findOne as jest.Mock).mockResolvedValueOnce(mockTransaction);
      await expect(
        transactionService.refundTransaction('user-1', 'trans-1'),
    ...
    it('deve lançar erro se a transação já foi estornada', async () => {
      const mockTransaction = { status: 'REVERSED', fromWallet: { user: { id: 'user-1' } } } as any as Transaction;
      (mockEntityManager.findOne as jest.Mock).mockResolvedValueOnce(mockTransaction);
      await expect(
        transactionService.refundTransaction('user-1', 'trans-1'),
    ...
    it('deve lançar erro se passou dos 7 dias', async () => {
      ...
      const mockTransaction = { status: 'SUCCESS', createdAt: oldDate, fromWallet: { user: { id: 'user-1' } } } as any as Transaction;
      (mockEntityManager.findOne as jest.Mock).mockResolvedValueOnce(mockTransaction);
      await expect(
        transactionService.refundTransaction('user-1', 'trans-1'),
    ...
    it('deve marcar REVERSED_FAILED e lançar exceção se saldo do toWallet for insuficiente', async () => {
      const mockTransaction = {
        ...
        createdAt: new Date(),
        amount: 500,
        fromWallet: { id: 'wallet-from', balance: 1000, user: { id: 'user-1' } },
        toWallet: { id: 'wallet-to', balance: 100, user: { id: 'user-2' } },
      } as any as Transaction;
      ...
      expect(mockEntityManager.save).toHaveBeenCalledWith(
        Transaction,
        expect.objectContaining({ status: 'REVERSED_FAILED' }),
      );
      expect(mockQueryRunner.commitTransaction).toHaveBeenCalled();
